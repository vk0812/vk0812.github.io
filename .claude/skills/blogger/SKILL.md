...
**Tone**
- Third-person or impersonal, conversational, and warm. Explain the system like you're talking to a smart friend over coffee, not lecturing. Do not use first-person narration such as "I will", "we will", "we can", "our system", or "this gives us" in blog prose.
...
- Use contractions ("it's", "that's", "don't"). Stiff prose feels off.
...
- This is a light-touch rule, not a mandate to rewrite every paragraph. Most paragraphs in a well-drafted post already satisfy it. Apply it as a self-check on the sections that feel the densest (a definition-heavy paragraph, a paragraph introducing two or three related mechanisms at once) rather than mechanically rewriting sentences that are already clear.
- Before handing back a new post, or when asked to simplify an existing one, reread it once specifically for this, dense/wordy sentences, jumped ideas, chained clauses, and fix only what actually needs it.

**Reference images, important**
- If the user supplies a reference image, inspect it with the available image-reading tool BEFORE writing anything. For a local image in Codex, use `view_image`. Extract the exact example used (question text, answer values, formula variant, table entries) and state it back in a short confirmation message so the user can catch misreads early.
...
**Section headings are sentence case, not Title Case.** A `Heading` reads "What must be true" or "Why read the log instead of the table", not "What Must Be True" or "Why Read The Log Instead Of The Table". Only the first word plus any proper noun or canonical acronym (CDC, SQL, GPU) is capitalized. This is separate from the post `title` field, which is also sentence case. Title Case section headers read as AI-generated and match no existing post, grep new `Heading` text and fix any that capitalize every word before handing back.

**Not every post is a case study, and the two shapes differ.** A "design X" case study (tag `Case Studies`) always has a capacity/sizing section (`CapacityMathDiagram` + `StatTiles`) and always ends with the animated final-architecture build-up. A topic explainer (tag `System Design`, e.g. caching, rate limiting, change data capture) usually has neither. Do NOT force a sizing section onto a concept post that isn't about scale, and do NOT add a grand "putting it all together" architecture diagram to a topic post, especially when the user asks you not to. A topic post ends on its Takeaways plus closing paragraph the way `rate-limiting.tsx` and `data-partitioning.tsx` do. A small static mid-post `IconArchitectureDiagram` (no `phases`) that shows one specific flow is still fine in a topic post, it's only the cumulative everything-at-once finale that's a case-study-only convention.
...
## Writing several posts at once (parallel subagents)

When the task is a batch (several posts in one go), the fast and safe pattern is one orchestrator plus one subagent per post, run in parallel. It has one hard rule that keeps the run from corrupting itself.

- **Subagents write ISOLATED files only.** Each agent creates exactly its own `src/content/posts/<slug>.tsx` and, if it needs animations, its own `src/content/components/animations/<slug>/ConceptViz.tsx`. Nothing else.
- **Subagents never touch the three shared registration files** (`src/content/posts/index.ts`, `src/content/components/index.ts`, `src/pages/Writings.tsx`). Parallel agents editing the same shared file clobber each other, the last writer wins and the rest are silently lost. The orchestrator wires all three itself, once, after the agents finish, using the export names each agent reports back.
- **Each agent self-verifies in isolation** (capacity math via `node -e`, `scripts/check-icon-architecture-diagram.py` for every `IconArchitectureDiagram` (not just large ones, see below), `scripts/check-svg-layout.py` with a `"canvas"` field for bespoke SVG, the delay staircase, the punctuation grep). A batch of 4 posts once shipped two visual bugs that these checks would have caught had they been run correctly, a wrapped `sub` label overlapping the node below it, and a bespoke animation's longest narration string and a box both clipping past their own `viewBox`, see the "Real incident" notes under "Building bespoke animations" and the `IconArchitectureDiagram` section. Neither post had 8+ nodes, so don't gate these checks on node count, run them on every diagram and every bespoke animation, every time. The one expected leftover from `npx tsc --noEmit` is the missing-barrel-export error for that post's own animation, which the wiring step resolves. Every other tsc error is the agent's to fix before it reports done. Have each agent end with a fixed report block listing its slug, post export name, animation file, and exact animation export names, so the orchestrator can wire without re-reading everything.
- **After wiring, the orchestrator runs the full check once** (`npx tsc --noEmit -p tsconfig.app.json`, then `npx vite build`, then a `curl` of each new `/writings/<slug>` route for a 200) and reads each finished post before calling it done. Use the Sonnet model for the writing agents, and give each one this SKILL.md plus the freshest existing case study to read as the reference.
...
**Actively look for visual teaching opportunities.** A long technical section that introduces a state progression, a structured data model, capacity math, a request path, a comparison, or a multi-service relationship should be evaluated for an existing code-built figure. Use a visual when it makes the mechanism easier to understand at a glance, not as decoration. A state progression can use a small static `IconArchitectureDiagram`, structured fields can use `SchemaCards`, endpoint contracts can use `ApiEndpointsTable`, and estimates can use `CapacityMathDiagram` and `StatTiles`. Add the figure directly after the paragraph that introduces the mechanism. Skip it when prose already makes the point more clearly or when the resulting layout would be cramped.

**Favor a few useful visuals over a wall of diagrams.** Each visual must introduce a distinct mental model or compress genuinely parallel information. Do not repeat the same request path in several figures. Keep early explanation figures static unless movement itself explains the idea. The final architecture is the deliberate exception. It always uses a phased `IconArchitectureDiagram` build-up.
...
**Real incident, don't hand-build a bespoke SVG for a plain node/edge system diagram, even a big "final architecture" one.** The Designing YouTube post's first draft built three separate one-off `animations/designing-youtube/ConceptViz.tsx` diagrams (a control/data path split, an encoding pipeline, and a full "putting it all together" diagram) using raw `<line>`/`foreignObject` boxes, none of them had per-edge labels, curves, or bidirectional arrows, so none of them needed to be bespoke, `IconArchitectureDiagram` covers every one of those shapes. The result read as visually inconsistent with every other case study on the site (different box style, different arrow stroke weight against the same `stroke-foreground/60` color) and had to be torn out and rebuilt as `IconArchitectureDiagram` calls, two small static ones early (`height` 300-420, no `phases`) plus one large animated one at "Putting it all together" (`height` 700-800, with `phases`), matching the exact pattern `designing-dropbox.tsx` and `designing-messenger.tsx` already use. Default to `IconArchitectureDiagram` for a "final complete design" figure even when it has 15+ nodes, don't reach for a hand-coded SVG just because the node count is large, only the presence of curves, edge labels, or double-headed labeled arrows justifies going bespoke (see the rule right above this one).

**A case study's final system-design overview is always animated (topic posts skip the finale entirely, see "Not every post is a case study" above).** Use `IconArchitectureDiagram` with cumulative `phases` for the final architecture figure. Start with the request entry point, then reveal the main services, durable stores, derived indexes, and downstream workers in the order the prose introduced them. Give each phase a short sentence that explains the newly visible relationship. The final figure should be a build-up, not a static dump of every box at once. Run the offline collision check on the complete node and edge set, including every phase's edges.

**Every visual must work in both themes.** All SVG animations and diagrams live inside a `.viz` wrapper and use the existing adaptive blueprint palette from `src/index.css`. Use `var(--v-ink)` for text, outlines, and neutral arrows, `var(--v-blue)` for positive structure or emphasis, `var(--v-blue-soft)` for ordinary highlighted boxes, `var(--v-panel)` for quiet panels, `var(--v-warn)` for real warnings or dense/problem states, and `var(--v-muted)` for secondary text. Prefer the semantic classes `viz-box`, `viz-panel`, `viz-panel-warn`, `viz-label`, `viz-label-sm`, `viz-phase`, `viz-thin`, `viz-stroke`, `viz-blue`, `viz-warn`, `viz-arrow-ink`, `viz-arrow-blue`, and `viz-arrow-warn`. Never use literal black, white, gray, or one-theme-only Tailwind colors for SVG fills, strokes, or text. Never invent a `--v-*` token in a component. If a genuinely new semantic color is necessary, add both light and dark values in `src/index.css`, add a semantic class, and document why it belongs in the fixed palette before using it.

**`IconArchitectureDiagram` node footprint scales with the `height` prop, not just node count, get this wrong and labels overlap or clip.** Each node's rendered box is roughly a fixed *pixel* size regardless of the diagram (icon +label(s), about 88px tall with no `sub`, about 102px tall with a `sub` line, about 112px wide), but node coordinates are percentages of `height`. That means the same node takes up a bigger percentage of a short diagram than a tall one. Concretely, in percent-of-height units, a node's half-height is `~4400/height` (no `sub`) or `~5100/height` (with `sub`), and half-width is a roughly constant `~8` (percent of the container's width, which doesn't vary with `height`). Two vertically stacked nodes need their y-values at least `(halfH_a + halfH_b)` apart or their boxes overlap, two side-by-side nodes need x-values at least 16 apart (`8+8`) for the same reason. A real incident, the Designing YouTube post's first `IconArchitectureDiagram` pass used a `height` too small for how many chain levels it had (a 6-node, 5-level chain at `height={420}`, and a top-row `client`/`lb`/`gateway` stack at `height={320}`), and BOTH shipped with real overlaps, labels clipping into the box above, and one node's label overflowing past the container's bottom edge entirely. Before picking `height` for a new diagram, count the deepest chain of nodes (client to leaf) and work backward, each level needs roughly 20 percent-of-height units of clearance (more if either neighboring node has a `sub` line), so a 5-level-deep diagram needs `height >= 5 * 20 * (44/0.20)`, or just estimate as `depth * ~90px` and round up generously. When in doubt, use a bigger `height` than feels necessary, extra empty space at the bottom of the container is harmless, cramped labels are not.

**The `~5100/height` "with sub" constant assumes the `sub` string renders on exactly one line, a longer `sub` silently invalidates it, real incident.** A node's `sub` sits inside the same fixed-width box as its label (96px wide at the mobile breakpoint, the tighter of the two), so a `sub` longer than roughly 18 characters wraps to two lines, and past roughly 35 characters it can wrap to three. The mixture-of-experts-and-sparse-models post shipped with a `combine` node whose `sub` read "router weight times expert output" (34 characters), which wrapped to three lines, made the node visibly taller than the formula assumed, and overlapped the node below it, a bug the old "write a throwaway Node script" check missed entirely because every throwaway script (written fresh per diagram, by different authors) assumed one line without ever questioning it. Keep `sub` text to roughly 18 characters or fewer so it reliably stays on one line, and if a longer `sub` is genuinely necessary, don't hand-wave the extra height, run the script below, which computes the real wrapped line count from the actual string instead of assuming.

**Verify every `IconArchitectureDiagram` with `scripts/check-icon-architecture-diagram.py`, not a throwaway script, before shipping.** Don't reach for a browser or Playwright/Chromium to "look at it", the user has explicitly said no to that, and don't hand-roll a fresh Node script per diagram either, that's exactly what let the wrapping bug above ship undetected. Build a small JSON mirroring the component's real props (`height`, and every node's `id`, `label`, `sub`, `x`, `y`, plus `edges`) in the scratchpad directory, then run:

```bash
python3 scripts/check-icon-architecture-diagram.py /tmp/my-diagram-layout.json
```

This script computes each node's TRUE footprint from the actual `label` and `sub` strings (estimating wrapped line count with `scripts/estimate_text_width.py` at both of the component's real Tailwind breakpoints, not a flat single-line assumption), then checks for node-node overlaps, edges crossing unrelated nodes, and any `sub` that wraps to three or more lines (a hard signal to shorten it or move the detail into prose instead). Run this for every `IconArchitectureDiagram`, not just ones with 8 or more nodes or a fan-out, the mixture-of-experts overlap above happened on a 5-level chain that would not have hit the old node-count threshold. Iterate coordinates or shorten text until it reports zero issues, then apply the final numbers to the actual `.tsx` file.

**Check every custom visual for collisions AND for staying inside its own viewBox, not just large architecture diagrams.** Before shipping a hand-coded SVG or bespoke GSAP animation, create a temporary JSON layout in the scratchpad with the boxes, labels, and connector points in the visual's real coordinate system, **plus a `"canvas": {"width": W, "height": H}` matching the component's `viewBox="0 0 W H"` exactly**, then run:

```bash
python3 scripts/check-svg-layout.py /tmp/my-visual-layout.json
```

The general checker reports overlapping boxes, labels touching boxes or other labels, connector endpoints placed inside boxes, connector segments crossing unrelated boxes, and (with `"canvas"` present) any box or label that extends past the viewBox itself. A straight arrow must begin and end at the outside boundary of its intended boxes, not at their centers or inside their text. Use a polyline only when the visual intentionally needs bends. Otherwise keep the original straight connector and fix the endpoint coordinates with simple geometry. The command must report zero issues before the coordinates are copied into the component. This is an offline collision check, so it does not require a browser, screenshot, Chromium, or Playwright.

**Real incident, an SVG has no overflow by default, content outside its viewBox just clips, and the old check never noticed because it had no notion of the canvas at all.** The mixture-of-experts-and-sparse-models post's bespoke token-routing animation shipped with two separate instances of this. First, its rightmost expert box was positioned and sized such that its right edge landed at x=925 inside a `viewBox="0 0 900 520"`, a plain, static, fully knowable overflow that nothing caught because `check-svg-layout.py` at the time only compared elements to each other, never to the canvas they're supposed to fit inside, that gap is now closed by the `"canvas"` field above. Second, and more subtly, the animation's GSAP timeline swaps a long narration sentence into a `text-anchor="middle"` element every few seconds (the "phase" pattern every bespoke animation on this site uses), and the offline check that ran before shipping only ever looked at the SVG's static initial frame, an empty string, so it never measured any of the sentences the timeline actually displays. The longest one, at roughly 100 characters in a bold, letter-spaced 14-unit font, rendered wider than the entire 900-unit canvas and clipped off both edges once scrolled into view. **Whenever a GSAP timeline assigns more than one string to the same text element over its lifetime (any "phase" narration, a running counter, anything read from an array and swapped in), include a label entry in the layout JSON for the LONGEST string the timeline ever assigns, not the empty initial value**, positioned at that element's real `x`/`y`/anchor, with its width estimated by `scripts/estimate_text_width.py` (see below). Checking only the first frame is checking the wrong frame.

**Estimate SVG text width with `scripts/estimate_text_width.py`, don't guess by eye and don't assume a sentence "probably fits."** This site's bespoke SVGs set `font-size` in the same user-coordinate units as everything else in the `viewBox` (a 14-unit font in a 900-unit-wide canvas, not 14 real screen pixels), so a long, bold, letter-spaced narration sentence can genuinely be wider than the canvas itself, exactly what happened in the incident above. The script gives a deliberately conservative (wider than most real fonts render) character-count estimate for every text class already defined in `src/index.css` and for `IconArchitectureDiagram`'s label/sub roles:

```bash
python3 scripts/estimate_text_width.py "Some narration string" --preset viz-phase
python3 scripts/estimate_text_width.py "Some narration string" --preset viz-phase --box-width 900   # also reports wrap count
```

Use it for two things before wiring coordinates into a component. First, whenever a `text-anchor="middle"` element sits at some `x` inside a canvas of width `W`, confirm the estimated width is comfortably less than `2 * min(x, W - x)`, not just barely less, leave real margin (a good rule of thumb is keeping the widest narration string under about 80% of the canvas width when centered, the incident above shipped at over 100%). Second, feed every candidate `sub`/`label` string and its real box width into `--box-width` to get a wrap count before deciding whether it fits on one line, rather than eyeballing character count. If a narration sentence doesn't fit with real margin, shorten it, a shorter sentence that fits is always better than a longer one that clips.

**A straight-line-only diagram can't route around clutter, so don't force an edge that has to travel back across a busy diagram diagonally.** `IconArchitectureDiagram` only draws direct point-to-point lines, no bends, no bezier routing. A "loop back to the client" edge from a node buried deep in a wide fan-out (a notification service, a CDN, anything positioned far from the top of the diagram) will frequently have to cross straight through unrelated nodes sitting between it and the client at the top. If the collision check above flags this and no reasonable repositioning fixes it, drop that edge entirely rather than accepting an overlap, the prose almost always already explains that path in detail elsewhere in the post, so the diagram doesn't need to close every loop visually. This happened on the Designing YouTube post's final diagram, three separate "back to client" edges (notification service, feed generation, CDN) all had to be cut for exactly this reason.
...
**`StatTiles` rounds every displayed `value` to a whole number, so a decimal loses its fraction silently.** A tile with `value: 9.3` renders as `9`, `1.5` renders as `2`, `1.44` renders as `1`. When a headline number is fractional, pick a unit that makes it whole instead (`9K/s` off a `9` value not `9.3`, `1500 GB` not `1.5 TB`, `1440M/day` not `1.44B/day`) so the tile and the prose agree. This shipped as a real bug once, decimal stat values rendered rounded and silently disagreed with the capacity math above them.
...
5. If the post has ANY `IconArchitectureDiagram`, not just ones with 8+ nodes or a fan-out, run `python3 scripts/check-icon-architecture-diagram.py` on a JSON mirroring its real `height`/`nodes`/`edges` and confirm zero node overlaps, zero edge crossings, and no `sub` wrapping to 3+ lines before picking a final `height` and coordinates. See the "Real incident" note under the `IconArchitectureDiagram` section, a 5-level chain with no fan-out still shipped a real overlap because a `sub` label wrapped more than the old flat-height formula assumed.
6. If the post has any bespoke SVG, GSAP animation, or hand-positioned visual, run `python3 scripts/check-svg-layout.py` with a temporary JSON layout that includes a `"canvas"` field matching the component's real `viewBox`, and confirm zero box, label, endpoint, and connector collisions AND zero canvas-bounds violations. If the animation's timeline ever swaps more than one string into the same text element (any "phase" narration), include the LONGEST string it ever assigns as a label, not just the empty initial value, estimate its width with `scripts/estimate_text_width.py` first. See "Real incident" under "Check every custom visual for collisions" above, checking only the first frame is checking the wrong frame.
7. Grep the file for stray `:`, `;`, `—`, `–` in prose one more time (see Punctuation rule above), it's cheap and catches anything a mid-edit slipped in.
8. Send the user the local URL.
9. **Don't commit.** Leave commits to the user unless they explicitly ask.

## Delay maintenance utility

When a post gains or loses paragraphs, headings, lists, or inline figures, use the repository utility to rebuild the normal content delay staircase:

```bash
python3 scripts/renumber-blog-delays.py src/content/posts/<slug>.tsx
```

The script assigns `0.1`, `0.15`, `0.2`, and so on to normal content in source order. Viewport-triggered figures, including `StatTiles`, `IconArchitectureDiagram`, `CapacityMathDiagram`, `ApiEndpointsTable`, `SchemaCards`, `ReplicationDiagram`, and custom components whose names end in `Diagram`, keep their independent `0.05` delay. Review the resulting sequence and run the normal type check and build afterward.
