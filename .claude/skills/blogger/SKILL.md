...
**Verify a many-node `IconArchitectureDiagram` layout with an offline math check before shipping, don't eyeball coordinates and don't reach for a browser or Playwright/Chromium to "look at it" (the user has explicitly said no to that approach, keep the check to pure math).** For any diagram with roughly 8 or more nodes, or any diagram with a fan-out (one node with 3+ children) or a diagonal edge crossing other rows, write a small throwaway Node script (in the scratchpad directory, not the repo) that mirrors the node/edge arrays with their real `x`, `y`, `height`, and `sub` values, computes each node's bounding box using the footprint formula above, and checks two things, whether any edge's straight line (clipped at both ends by the component's `NODE_R=8` percent radius, same as the component does) passes through a node box it doesn't connect to, and whether any two node boxes themselves overlap. Iterate coordinates until the script reports zero problems, then apply the final numbers to the actual `.tsx` file. This catches exactly the class of bug in the paragraph above without needing to render anything. Delete the throwaway script when done, it's scratch work, not something to leave in the repo.

**Check every custom visual for collisions, not just large architecture diagrams.** Before shipping a hand-coded SVG, create a temporary JSON layout in the scratchpad with the boxes, labels, and connector points in the visual's real coordinate system, then run:

```bash
python3 scripts/check-svg-layout.py /tmp/my-visual-layout.json
```

The general checker reports overlapping boxes, labels touching boxes or other labels, connector endpoints placed inside boxes, and connector segments crossing unrelated boxes. A straight arrow must begin and end at the outside boundary of its intended boxes, not at their centers or inside their text. Use a polyline only when the visual intentionally needs bends. Otherwise keep the original straight connector and fix the endpoint coordinates with simple geometry. The command must report zero issues before the coordinates are copied into the component. This is an offline collision check, so it does not require a browser, screenshot, Chromium, or Playwright.
...
5. If the post has an `IconArchitectureDiagram` with roughly 8 or more nodes, any fan-out, or any long/diagonal edge, run the offline collision check described above (throwaway Node script, no browser) and confirm zero overlaps before picking a final `height` and coordinates.
6. If the post has any bespoke SVG or hand-positioned visual, run `python3 scripts/check-svg-layout.py` with a temporary JSON layout and confirm zero box, label, endpoint, and connector collisions.
