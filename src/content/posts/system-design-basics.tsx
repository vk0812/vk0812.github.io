...
import { Clock, Users, Waypoints, Server, Database } from "lucide-react";

const stackNodes: DiagramNode[] = [
  { id: "client", label: "Client", icon: Users, color: "text-slate-500", x: 50, y: 6 },
  { id: "lb", label: "Load Balancer", icon: Waypoints, color: "text-blue-500", x: 50, y: 32 },
  { id: "app1", label: "App A-1", icon: Server, color: "text-purple-500", x: 20, y: 60 },
  { id: "app2", label: "App A-2", icon: Server, color: "text-purple-500", x: 50, y: 60 },
  { id: "app3", label: "App A-3", icon: Server, color: "text-purple-500", x: 80, y: 60 },
  { id: "dbPrimary", label: "Primary DB", icon: Database, color: "text-blue-600", x: 35, y: 90 },
  { id: "dbReplica", label: "Replica DB", icon: Database, color: "text-indigo-500", x: 65, y: 90 },
];

const stackEdges: DiagramEdge[] = [
  { id: "e1", from: "client", to: "lb" },
  { id: "e2", from: "lb", to: "app1" },
  { id: "e3", from: "lb", to: "app2" },
  { id: "e4", from: "lb", to: "app3" },
  { id: "e5", from: "app1", to: "dbPrimary" },
  { id: "e6", from: "app2", to: "dbPrimary" },
  { id: "e7", from: "app3", to: "dbPrimary" },
  { id: "e8", from: "dbPrimary", to: "dbReplica" },
];
  ...
  content: (
    <>
        ...
        panels={[
          {
            ...
            writeLabel: "Incoming traffic",
            fanLabel: "distributed across nodes",
            nodes: ["Node 1", "Node 2", "Node 3"],
            note: "Add more machines as load grows, no single node has to get bigger.",
          },
          {
            title: "Vertical scaling",
            writeLabel: "Incoming traffic",
            fanLabel: "handled by one bigger machine",
            nodes: ["Single upgraded node"],
            note: "Give the existing machine more CPU, RAM, or storage, until it hits a physical ceiling.",
          },
        ]}
      />
      ...
      <StatTiles
        delay={0.05}
        items={[
          { label: "Downtime per year at 99% uptime", value: 5256, suffix: " min", icon: Clock, color: "text-rose-500" },
          { label: "Downtime per year at 99.9% uptime", value: 526, suffix: " min", icon: Clock, color: "text-amber-500" },
          { label: "Downtime per year at 99.99% uptime", value: 53, suffix: " min", icon: Clock, color: "text-blue-500" },
          { label: "Downtime per year at 99.999% uptime", value: 5, suffix: " min", icon: Clock, color: "text-emerald-600" },
        ]}
      />
      ...
      <IconArchitectureDiagram
        ...
        height={420}
        delay={0.05}
        caption="The basic scalable stack. A load balancer fans requests out across app server replicas, and a primary database replicates to a standby so one failure does not take the whole system down."
      />
