...
import {
  ...
  DiagramNode,
  DiagramEdge,
  HeartbeatPulseDiagram,
} from "../components";
import { Database, Download, Hash, GitCompare, CheckCircle2, XCircle } from "lucide-react";

const checksumNodes: DiagramNode[] = [
  { id: "stored", label: "Data + checksum stored", icon: Database, color: "text-blue-600", x: 50, y: 10 },
  { id: "request", label: "Client requests data", icon: Download, color: "text-slate-500", x: 50, y: 32 },
  { id: "recompute", label: "Recompute SHA-256", icon: Hash, color: "text-violet-500", x: 50, y: 54 },
  { id: "compare", label: "Compare checksums", icon: GitCompare, color: "text-slate-500", x: 50, y: 76 },
  { id: "match", label: "Match, data intact", icon: CheckCircle2, color: "text-emerald-600", x: 25, y: 96 },
  { id: "mismatch", label: "Mismatch, corrupted", icon: XCircle, color: "text-red-600", x: 75, y: 96 },
];

const checksumEdges: DiagramEdge[] = [
  { id: "stored-request", from: "stored", to: "request" },
  { id: "request-recompute", from: "request", to: "recompute" },
  { id: "recompute-compare", from: "recompute", to: "compare" },
  { id: "compare-match", from: "compare", to: "match" },
  { id: "compare-mismatch", from: "compare", to: "mismatch" },
];
...
export const heartbeatAndChecksum: BlogPostData = {
  ...
  content: (
    <>
      ...
      <HeartbeatPulseDiagram
        delay={0.08}
        caption="Server A's periodic heartbeat to the Load Balancer, then silence, then a timeout flip to failed and a reroute to Server B."
      />
      ...
      <IconArchitectureDiagram
        ...
        height={520}
        delay={0.08}
        caption="The checksum flow. A hash is written alongside the data, recomputed on read, and compared, a mismatch forks into a rejected read."
      />
