...
export const redundancyAndReplication: BlogPostData = {
  ...
  content: (
    <>
      ...
      <SyncAsyncTimingDiagram
        delay={0.08}
        caption="Synchronous replication only acks the client after the replica confirms, asynchronous replication acks right away and replicates in the background."
      />
      ...
      <ReplicationDiagram
        ...
        panels={[
          {
            title: "Single-leader (MySQL primary-replica)",
            writeLabel: "Write to primary",
            fanLabel: "replicates to followers",
            nodes: ["Primary", "Follower", "Follower"],
            highlightNodes: [0],
            note: "One leader orders every write. Simple, but it's a bottleneck and a single point of write failure.",
          },
          {
            title: "Multi-leader (CouchDB bidirectional sync)",
            writeLabel: "Write to any leader",
            fanLabel: "syncs bidirectionally",
            nodes: ["Leader (Tokyo)", "Leader (Virginia)", "Leader (London)"],
            highlightNodes: [0, 1, 2],
            note: "Every leader accepts writes locally. Conflicting writes still need something to decide which one wins.",
          },
        ]}
      />
