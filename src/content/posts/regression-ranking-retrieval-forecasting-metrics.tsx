  ...
  content: (
    <>
      ...
      <Paragraph delay={0.15}>
        That choice is the actual subject here, not the formulas themselves. Regression, ranking, retrieval, and forecasting all reduce to "how far off was the model," but each one asks that question differently, and picking the metric that matches how the prediction gets used, not the one that's easiest to compute or the most familiar from a stats class, is most of the job.
      </Paragraph>
...
# 0.54 0.358 0.598 0.941`}
      ...
      <Paragraph delay={0.85}>
        For the five points above that ratio comes out to <Formula>{`0.941`}</Formula>, the model explains 94 percent of the spread in the actual values. It's a genuinely useful single number for "does this model track the data at all," and it has three limits that get ignored constantly. It never decreases when a new feature gets added, regardless of whether that feature means anything, so comparing models with different feature counts on raw R-squared rewards padding the input list. It says nothing about the actual size of the error in the target's own units, a model can post a high R-squared purely because the target barely varies to begin with, not because the predictions are especially accurate. And it's computed on whatever data it was handed, a strong R-squared on training data is not a claim about how the model performs once it sees something new.
      </Paragraph>
...
print(abs(actual - 1000.0) / actual * 100)  # 900.0, prediction of 1000`}
      ...
      <Paragraph delay={1.75}>
        Twenty documents in a corpus are genuinely relevant to a query, and the first-stage retriever's top ten results contain six of them. Recall@10 is <Formula>{`6/20 = 0.3`}</Formula>, regardless of whether those six landed at ranks one through six or were scattered across the full ten, order plays no role at all. That's the deliberate difference from NDCG, a retrieval stage is usually followed by a separate ranking stage that will sort whatever made the shortlist, so its own job is just to not leave a relevant item out of the shortlist entirely. A retriever tuned to maximize precision instead of recall can look excellent while quietly discarding items the downstream ranker never gets a chance to consider, which is why recall, not precision, is usually the metric that matters most at this stage of the pipeline.
      </Paragraph>
      ...
      <Paragraph delay={1.85}>
        A different failure mode shows up once a model's output is a probability rather than a point estimate, a 70 percent chance of rain, a 70 percent predicted likelihood of churn, a quantile interval meant to contain the true value 90 percent of the time. None of MAE, RMSE, or NDCG check whether those stated probabilities are honest. <strong>Calibration</strong> does, it asks a narrower question, of everything the model called "70 percent likely," did roughly 70 percent of those events actually happen. A model whose "70 percent" events happen 90 percent of the time is systematically underconfident, and one whose "70 percent" events happen only 40 percent of the time is systematically overconfident, and a model can be well calibrated overall while still ranking and discriminating between cases poorly, or vice versa, calibration and discriminative accuracy are genuinely separate properties.
      </Paragraph>
