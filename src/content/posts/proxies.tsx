...
const forwardProxyNodes: DiagramNode[] = [
  { id: "c1", label: "Client A", icon: Users, color: "text-slate-500", x: 10, y: 20 },
  { id: "c2", label: "Client B", icon: Users, color: "text-slate-500", x: 10, y: 50 },
  { id: "c3", label: "Client C", icon: Users, color: "text-slate-500", x: 10, y: 80 },
  { id: "proxy", label: "Forward proxy", icon: Shield, color: "text-violet-500", x: 45, y: 50 },
  { id: "dest", label: "Internet", icon: Globe, color: "text-blue-600", x: 80, y: 50 },
];

const forwardProxyEdges: DiagramEdge[] = [
  { id: "e1", from: "c1", to: "proxy" },
  { id: "e2", from: "c2", to: "proxy" },
  { id: "e3", from: "c3", to: "proxy" },
  { id: "e4", from: "proxy", to: "dest" },
];

const reverseProxyNodes: DiagramNode[] = [
  { id: "client", label: "Client requests", icon: Globe, color: "text-slate-500", x: 10, y: 50 },
  { id: "proxy", label: "Reverse proxy", icon: Shield, color: "text-violet-500", x: 45, y: 50 },
  { id: "s1", label: "Server A", icon: Server, color: "text-blue-600", x: 80, y: 20 },
  { id: "s2", label: "Server B", icon: Server, color: "text-blue-600", x: 80, y: 50 },
  { id: "s3", label: "Server C", icon: Server, color: "text-blue-600", x: 80, y: 80 },
];

const reverseProxyEdges: DiagramEdge[] = [
  { id: "e1", from: "client", to: "proxy" },
  { id: "e2", from: "proxy", to: "s1" },
  { id: "e3", from: "proxy", to: "s2" },
  { id: "e4", from: "proxy", to: "s3" },
];
  ...
  content: (
    <>
      ...
      <IconArchitectureDiagram
        ...
        edges={forwardProxyEdges}
        height={340}
        delay={0.05}
        caption="Clients route through a single forward proxy to reach the internet, so the destination only ever sees the proxy."
      />
      ...
      <IconArchitectureDiagram
        nodes={reverseProxyNodes}
        edges={reverseProxyEdges}
        height={340}
        delay={0.05}
        caption="Incoming requests hit the reverse proxy, which fans them out across backend servers the client never sees directly."
      />
